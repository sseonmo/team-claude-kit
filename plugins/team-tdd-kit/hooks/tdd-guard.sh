...
#     (latest.ts, contest.ts 처럼 이름에 test 가 들어간 구현 파일이 통과되던 문제)
#   - 거부 메시지의 확장자를 .ts 고정에서 실제 확장자로 변경
      ...
      cat << EOF
{
  "hookSpecificOutput": {
    ...
    "permissionDecisionReason": "TDD GUARD: '${BASENAME}'에 대한 테스트 파일이 존재하지 않습니다. 구현 코드를 작성하기 전에 테스트를 먼저 작성하세요. (테스트 파일 예: ${BASENAME}.test.${EXT_ACTUAL})"
  }
}
EOF
    fi
