...
```bash
SCORE=$(find "$HOME/.claude/plugins" "$HOME/.claude/skills" \
  -path '*ai-readiness-cartography/scripts/score.py' 2>/dev/null | head -1)
python3 "$SCORE" <repo-path> --json <output-path>/ai-readiness-score.json
```

`SCORE` 가 비면 스킬이 제대로 설치되지 않은 것이다 — 사용자에게 알리고 중단한다.
